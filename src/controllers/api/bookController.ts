...
class BookController {
	...
	async getBookById(req: Request, res: Response) {
		...
		if (!newBook) {
			...
			res.json({ ...newBook.toObject(), count: howMany.data.count });
		}
	}
	...
	async downloadBookFile(req: Request, res: Response) {
		try {
			...
			let filePath;
			const { id } = req.params;
			const book: IBookDoc | null = await Book.findOne({ _id: id });
			if (book && filename) {
	...
	async deleteBookWithFile(req: Request, res: Response) {
		...
		try {
			...
			await bookContainer.deleteBook(id); // удаление книги из базы данных

			if (findBook.fileName) {
				const filePath = path.join(__dirname, '../../db/fileBooks', path.basename(findBook.fileName));

				try {
					await fs.unlink(filePath, (err) => {
						if (err)
							console.log({ message: `Ошибка при удалении файла: ${err}` });
